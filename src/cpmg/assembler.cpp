...
int
assemble2DDiffusionConvectionProblem(
        const pybind11::array_t<bool> & boundaryMask,
...
)
{
    ...
    auto f = laplacian.unchecked<kDim>();
    auto b = boundaryValue.unchecked<kDim>();
    auto vvx = vx.unchecked<kDim>();
    auto vvy = vy.unchecked<kDim>();
    ...
    for (int i = 0, k = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++j, ++k)
        {
            ...
            else
            {
                // Interior point. Use 5-point finite difference stencil.
                col.emplace_back(k - n);
                val.emplace_back(-alpha + 0.5 * vvy(i, j));

                col.emplace_back(k - 1);
                val.emplace_back(-alpha - 0.5 * vvx(i, j));

                col.emplace_back(k);
                val.emplace_back(beta + 4.0 * alpha);
